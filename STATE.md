...
M2 (temporal integrity) is done — all four sub-checks in `mekiki.checks.temporal`, each with a clean control, precisely-injected defects at known magnitudes, and real-data verification. M3 (action-state consistency, the flagship check) is next, and per the roadmap it needs `docs/consistency.md` written first — same discipline `docs/episode.md` got before any Episode/Frame code, since M3 is explicitly the check the whole project is supposed to be judged on. Don't rush into code before that doc exists.
