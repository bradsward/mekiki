...
- [ ] M3 — action-state consistency (the flagship one). forward-integrate commanded actions, diff against recorded proprio, report residuals in real units. `docs/consistency.md` first — tolerance has to scale with control freq and robot type, don't rush it.
      ...
      - [x] residual + tolerance check itself — `check_action_state_consistency` + `ToleranceModel` (caller-supplied `base + rate*dt` per quantity, never invented). clean-control and known-magnitude-defect fixtures done; **not yet verified against real data** — see the recommendation in STATE.md, this is a real gap, not a formality left undone.
      - [ ] residual + tolerance check itself, with clean-control and known-magnitude-defect fixtures, verified against real data
